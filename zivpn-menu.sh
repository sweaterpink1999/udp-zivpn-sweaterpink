#!/bin/bash
# ZIVPN Menu - COLOR UI (MULTI PASS UP TO 80)
...
list_accounts() {
jq -r '.auth.config[]' "$CONFIG" | nl -w2 -s'. '
}
...
if [ "$COUNT" -ge 80 ]; then
  echo -e "${RED}Max 80 accounts reached!${NC}"
  sleep 2
  return
fi
...
delete_account() {
clear
echo -e "${WHITE}ACTIVE ACCOUNTS${NC}"
list_accounts
echo
read -p "Delete number : " NUM

jq "del(.auth.config[$((NUM-1))])" "$CONFIG" > /tmp/z.json && mv /tmp/z.json "$CONFIG"
systemctl restart zivpn
